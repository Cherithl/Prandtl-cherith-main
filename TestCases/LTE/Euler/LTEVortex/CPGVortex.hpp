...
std::function<void(const Vector&, Vector&)> CPGVortexIC(real_t radius,
                                                        real_t vel_inf,
                                                        real_t beta,
                                                        real_t rho_inf,
                                                        real_t rhoe_inf)
{
    return [=](const Vector &x, Vector &y)
    {
        ...
        const real_t rhoe = pres / gm1 + den*mix.mixtureHMass(0.0000001);
...
struct RegisterCPGVortex
{
    RegisterCPGVortex()
    {
        // Register initial condition.
        ConditionFactory::Instance().RegisterInitialCondition5("CPGVortexIC", CPGVortexIC);
    }
};
// Global static instance to ensure registration happens at startup.
static RegisterCPGVortex regCPGVortex;
